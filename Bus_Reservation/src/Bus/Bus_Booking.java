...
public class Bus_Booking {
			...
			public static void main(String[] args) {
				try(Connection conn= Data.DataBaseConnection.getConnection();
			...
			private static void viewBusInfo(Connection conn) throws SQLException{
				String query="SELECT * FROM busesbooking";
			 ...
			 private static void bookBus(Connection conn, Scanner sc) throws ParseException, SQLException{
			        ...
			        if (isBusAvailable(conn, busNo, startDate, endDate)) {
			            ...
			            String insertBooking = "INSERT INTO booking (Passenger_Name, Bus_No, Start_Date, End_Date, Total_Rent) VALUES (?, ?, ?, ?, ?)";
			        ...
			        private static boolean isBusAvailable(Connection conn, int busNo, Date startDate, Date endDate) throws SQLException {
			            String query = "SELECT * FROM booking WHERE Bus_No = ? AND (Start_Date <= ? AND End_Date >= ?)";
			            ...
			            String busQuery = "SELECT Bus_Available FROM busesbooking WHERE Bus_No = ?";
			        ...
			        private static double calculateRent(Connection conn, int busNo, long days) throws SQLException {
			            String query = "SELECT Base_Price FROM busesbooking WHERE Bus_No = ?";
			            try (PreparedStatement pstmt = conn.prepareStatement(query)) {
			                pstmt.setInt(1, busNo);
			                try (ResultSet rs = pstmt.executeQuery()) {
			                    if (rs.next()) {
			        ...
			        private static void markBusUnavailable(Connection conn, int busNo) throws SQLException {
			            String updateBus = "UPDATE busesbooking SET Bus_Available = FALSE WHERE Bus_No = ?";
			        ...
			        private static boolean busRented(Connection conn, int busNo) throws SQLException {
			            String query = "SELECT Bus_Available FROM busesbooking WHERE Bus_No = ?";
			            try (PreparedStatement pstmt = conn.prepareStatement(query)) {
			                pstmt.setInt(1, busNo);
			                try (ResultSet rs = pstmt.executeQuery()) {
			                    if (rs.next()) {
